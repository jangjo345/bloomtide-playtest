export function validateRecord(record) {
  ...
  if(!s||!Number.isInteger(s.schemaVersion)||s.schemaVersion<1||s.schemaVersion>5)throw new Error("지원하지 않는 저장 버전입니다. 최신 검토 링크로 열어 주세요.");
...
export class ReviewStore {
  ...
  async save(record,json) {
    ...
    await new Promise((resolve,reject)=>{
      ...
      let conflict=false;
      r.onsuccess=()=>{const old=r.result;if(!old||old.format!==1||old.revision!==record.revision){conflict=true;tx.abort();return;}s.put(next);};
